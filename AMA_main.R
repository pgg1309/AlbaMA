...
obs_imp_rf_center[, -1] <- obs_imp_rf_center[, -1] /
  matrix(
...
for (t in 2:nrow(data)) {
  ...
  for (tt in 1:length(rf_model$inbag.counts)) {
    ...
    # --- Get the in-sample indices that are assigned to 'leaf_ll' and that are NOT OOB:
    obs_ins_ll <- which(
      (rf_leaf_ins_tt == rf_leaf_oos_tt) & (rf_model$inbag.counts[[tt]]) == 1
    )
...
obs_imp_rf_right[, -1] <- obs_imp_rf_right[, -1] /
  matrix(
...
#' Get Table B-6 Order
#' Returns a character vector of industry names in Table B-6 order
