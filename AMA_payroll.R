...
# ------------------------------------------------
# --- Data Settings
labeling <- "Private Employment"
start.date <- "1963-01-01"

# --- Hyperparameter Settings (selected using tuneRanger)
N_trees <- 1000
min_node_size <- 3
sampsize_rate <- 0.4
...
estimateTimeTuneRanger(payroll_task)
# Tuning process
res <- tuneRanger(
    payroll_task,
    measure = list(medae),
...
obs_imp_rf_center[, -(1:2)] <- obs_imp_rf_center[, -(1:2)] /
    matrix(
...
obs_imp_rf_right[, -(1:2)] <- obs_imp_rf_right[, -(1:2)] /
    matrix(
...
plot1 <- data |>
    ...
    geom_line(
        aes(x = DATE, y = Albama_center, col = "Albama_center"),
        linewidth = 0.7
    ) +
    geom_line(
        aes(x = DATE, y = Albama_right, col = "Albama_right"),
        linewidth = 0.7
    ) +
    scale_color_manual(
        name = "",
        values = c(
            "MAIN" = "black",
            "Albama_center" = "navyblue",
            "Albama_right" = "firebrick"
        ),
        labels = c(
            "MAIN" = "MoM Series",
            "Albama_center" = "AlbaMA (two-sided)",
            "Albama_right" = "AlbaMA (one-sided)"
        ),
        na.translate = FALSE
    ) +
    xlab('') +
...
for (ma.type in c('center', 'right')) {
    ...
    if (ma.type == "right") {
        ...
        plot_imp_agg <- plot_imp %>%
            ...
            mutate(
                `y(t-3:end)` = rowSums(
                    select(., matches("^lag_(3|[4-9]|[1-9][0-9]+)$")),
                    na.rm = TRUE
                )
            ) %>%
            mutate(
                `y(t+3:end)` = rowSums(
                    select(., matches("^lag_-[3-9]|^lag_-[1-9][0-9]+$")),
                    na.rm = TRUE
                )
            ) %>%
            mutate(DATE = data$DATE)
